...
P2_3 VRY of Joystick
P2_2 VRX of Joystick
...
{
	printf("Command: %s", s);
	P1_6=0; // 'set' pin to 0 is 'AT' mode.
	waitms(5);
	sendstr1(s);
	getstr1(buff);
	waitms(10);
	P1_6=1; // 'set' pin to 1 is normal operation mode.
	printf("Response: %s\r\n", buff);
}


//Function below are for LCD
...
//functions below are for adc input 

#define VDD 3.3049 // The measured value of VDD in volts
...
void main (void)
{
	...
	//initialize pins to be used for joystick
	InitPinADC(2,3);
	InitPinADC(2,2);
	InitADC();
